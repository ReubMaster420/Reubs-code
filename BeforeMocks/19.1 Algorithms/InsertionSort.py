...
def insertion(list):
    for i in range(len(list)):
        ...
        while j >= 0 and list[j] > currentvalue: #makes sure the first run where j = -1 does not work and instead mores to j = 0, where the second part queies if the value above the j index is smaller
